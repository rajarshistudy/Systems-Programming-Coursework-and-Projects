...
void* mm_malloc (size_t size) {
  ...
  if (blockSize - reqSize >= MIN_BLOCK_SIZE) {
    // allocated block at the front 
    ptrFreeBlock->sizeAndTags = reqSize | TAG_USED | precedingBlockUseTag;

    // build the remainder as a free block 
    BlockInfo* remainder = (BlockInfo*)UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize);
    size_t remSize = blockSize - reqSize;

    remainder->sizeAndTags = remSize | TAG_PRECEDING_USED; // prev is allocated now 
    // write remainder footer 
    *((size_t*)UNSCALED_POINTER_ADD(remainder, remSize - WORD_SIZE)) =
        remSize | TAG_PRECEDING_USED;

    // the block after the remainder now follows a free block 
    BlockInfo* following = (BlockInfo*)UNSCALED_POINTER_ADD(remainder, remSize);
    following->sizeAndTags &= ~TAG_PRECEDING_USED;

    // push remainder into free list 
    insertFreeBlock(remainder);
  } else {
    // allocate whole block; set following’s PRECEDING_USED 
    ptrFreeBlock->sizeAndTags = blockSize | TAG_USED | precedingBlockUseTag;

    BlockInfo* following = (BlockInfo*)UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize);
    following->sizeAndTags |= TAG_PRECEDING_USED;
  }
...
void mm_free (void *ptr) {
  ...
  size_t size = SIZE(block->sizeAndTags);

  // mark block free (clear TAG_USED, keep TAG_PRECEDING_USED as is) 
  block->sizeAndTags = size | (block->sizeAndTags & TAG_PRECEDING_USED);

  // write boundary footer to enable coalescing 
  *((size_t*)UNSCALED_POINTER_ADD(block, size - WORD_SIZE)) =
      block->sizeAndTags;

  // following block now precedes a FREE block
  BlockInfo *following = (BlockInfo*)UNSCALED_POINTER_ADD(block, size);
  following->sizeAndTags &= ~TAG_PRECEDING_USED;

  // add to free list, then coalesce with neighbors 
...
void* mm_realloc(void* ptr, size_t size) {
  ...
  // Compute requested *block* size (payload + header), aligned, >= MIN_BLOCK_SIZE 
  size_t reqSize;
  size_t raw = size + WORD_SIZE;              
  if (raw <= MIN_BLOCK_SIZE) {
    reqSize = MIN_BLOCK_SIZE;
  } else {
    reqSize = ALIGNMENT * ((raw + ALIGNMENT - 1) / ALIGNMENT);
  }

  // Current block info 
  BlockInfo* block = (BlockInfo*)UNSCALED_POINTER_SUB(ptr, WORD_SIZE);
  size_t curSize   = SIZE(block->sizeAndTags);  
  size_t prevTag   = block->sizeAndTags & TAG_PRECEDING_USED;

  // Case 1: Shrink in place
  if (reqSize <= curSize) {
    size_t slack = curSize - reqSize;
    if (slack >= MIN_BLOCK_SIZE) {
      // Split tail into a free remainder 
      block->sizeAndTags = reqSize | TAG_USED | prevTag;

      BlockInfo* remainder = (BlockInfo*)UNSCALED_POINTER_ADD(block, reqSize);
      remainder->sizeAndTags = slack | TAG_PRECEDING_USED;  // preceding is allocated now 

      // write footer of remainder 
      *((size_t*)UNSCALED_POINTER_ADD(remainder, slack - WORD_SIZE)) =
          slack | TAG_PRECEDING_USED;

      // following block after remainder now precedes a FREE block 
      BlockInfo* following = (BlockInfo*)UNSCALED_POINTER_ADD(remainder, slack);
      following->sizeAndTags &= ~TAG_PRECEDING_USED;

      insertFreeBlock(remainder);
      coalesceFreeBlock(remainder);
    }
    return ptr;
  }

  // Case 2: Try to grow in place using following free block 
  BlockInfo* following = (BlockInfo*)UNSCALED_POINTER_ADD(block, curSize);
  if ((following->sizeAndTags & TAG_USED) == 0) { 
    size_t folSize = SIZE(following->sizeAndTags);
    size_t combined = curSize + folSize;

    if (combined >= reqSize) {
      // consume the following free block 
      removeFreeBlock(following);

      size_t newSlack = combined - reqSize;
      if (newSlack >= MIN_BLOCK_SIZE) {
        // grow current to reqSize, make remainder free 
        block->sizeAndTags = reqSize | TAG_USED | prevTag;

        BlockInfo* remainder = (BlockInfo*)UNSCALED_POINTER_ADD(block, reqSize);
        remainder->sizeAndTags = newSlack | TAG_PRECEDING_USED;

        // footer for remainder 
        *((size_t*)UNSCALED_POINTER_ADD(remainder, newSlack - WORD_SIZE)) =
            newSlack | TAG_PRECEDING_USED;

        // block after remainder now precedes a FREE block 
        BlockInfo* after = (BlockInfo*)UNSCALED_POINTER_ADD(remainder, newSlack);
        after->sizeAndTags &= ~TAG_PRECEDING_USED;

        insertFreeBlock(remainder);
        coalesceFreeBlock(remainder);  // merge forward if possible 
      } else {
        // take the whole combined region
        block->sizeAndTags = combined | TAG_USED | prevTag;

        BlockInfo* after = (BlockInfo*)UNSCALED_POINTER_ADD(block, combined);
        after->sizeAndTags |= TAG_PRECEDING_USED;  // follows an allocated block 
      }
      return (void*)UNSCALED_POINTER_ADD(block, WORD_SIZE); // same payload 
    }
  }
  ...
  // Copy min(oldPayload, newPayload) without memcpy: WORD_SIZE at a time 
  size_t oldPayload = curSize - WORD_SIZE;
  size_t newPayload = size;
  size_t n = oldPayload < newPayload ? oldPayload : newPayload;

  // copy full words 
  size_t words = n / WORD_SIZE;
  size_t i;
  size_t* srcW = (size_t*)ptr;
  size_t* dstW = (size_t*)newPtr;
  for (i = 0; i < words; i++) {
    dstW[i] = srcW[i];
  }
  // copy any remaining bytes
  size_t rem = n - words * WORD_SIZE;
  unsigned char* srcB = (unsigned char*)ptr + words * WORD_SIZE;
  unsigned char* dstB = (unsigned char*)newPtr + words * WORD_SIZE;
  for (i = 0; i < rem; i++) {
    dstB[i] = srcB[i];
  }
