...
{
    char *argv[MAXARGS];
    int bg = parseline(cmdline, argv);     /* split into argv; bg==1 if ends with & */
    if (argv[0] == NULL) return;           /* deals with empty line */
    ...
    /* Block SIGCHLD to avoid race with handler (child might exit fast) */
    sigprocmask(SIG_BLOCK, &mask, &prev);

    pid_t pid = fork();
    if (pid == 0) {
        /* Child process */
        /* Put child in its own process group so shell doesn’t get Ctrl-C/Z */
        setpgid(0, 0);

        /* Unblock SIGCHLD in child before exec */
        sigprocmask(SIG_SETMASK, &prev, NULL);

        /* Run program. If it fails, print required message and exit. */
    ...
    int state = bg ? BG : FG;
    addjob(jobs, pid, state, cmdline);     /* record child in job list */

    /* Safe to unblock now that job is recorded */
    sigprocmask(SIG_SETMASK, &prev, NULL);

    if (bg) {
        /* Background: just print and return to prompt */
        printf("[%d] (%d) %s", pid2jid(pid), pid, cmdline);
    } else {
        /* Foreground: wait until it’s no longer the FG job */
        waitfg(pid);
    }
    return;
}
